const data = `Game 1: 4 green, 2 blue; 1 red, 1 blue, 4 green; 3 green, 4 blue, 1 red; 7 green, 2 blue, 4 red; 3 red, 7 green; 3 red, 3 green
...
const data2 = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
const getColorIndexes = (color, str) => {
  const result = [];
  for(let i = 0; i < str.length; i++){
    if (str.substr(i, color.length) === color) {
      result.push(i);
    }
  }
  return result;
...
const getNumberByIndex = (index, str) => {
  const before = str.substring(0, index);
  let i = index - 1;
  let numberStr = '';
  while (i >= 0 && !isNaN(before[i])) { 
    numberStr = before[i] + numberStr;
    i--;
  }
  return parseInt(numberStr, 10);
}

const getGameNumber = (str) => {
  let numberIndex = 5;
  let numberStr = '';
  while (numberIndex < 9 && !isNaN(str[numberIndex])) {
    numberStr += str[numberIndex];
    numberIndex++;
  }
  return parseInt(numberStr, 10);
}
...
arrayData.forEach(str => {
  ...
  for(const colorI in colors) {
    ...
    let colorNumber = 0;
    const indexes = getColorIndexes(color, str);
    for(const i of indexes){
      const number = getNumberByIndex(i, str);
      colorNumber = colorNumber < number ? number : colorNumber;
    }
    if(colorNumber){
      if(!power) power = 1;
      power = power * colorNumber;
    }
...
//             return limits[c[1]] >= Number(c[0]);
//         });
//     });
//     console.log(game, isPossible ? gameId : 0);
//     return isPossible ? gameId : 0;
// }

// console.log(lines.reduce((sum, line) => sum + number(line), 0));