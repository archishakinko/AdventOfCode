const data = `......644............612.......254..638..............802.................................118.....................................317.691....
...
function findNumbersSurroundedBySymbols(arrayData) {
  const result = [];

  for (let i = 1; i < arrayData.length - 1; i++) {
    const regex = /[0-9]+/g;
    let match;
    while ((match = regex.exec(arrayData[i])) !== null) {
      const number = match[0];
      const startIndex = match.index;
      const endIndex = match.index + number.length - 1;

      let surroundedBySymbols = true;

      for (let j = startIndex - 1; j <= endIndex + 1; j++) {
        if (
          (arrayData[i - 1][j] !== undefined && !isNaN(parseInt(arrayData[i - 1][j], 10))) ||
          (arrayData[i + 1][j] !== undefined && !isNaN(parseInt(arrayData[i + 1][j], 10))) ||
          (j !== startIndex &&
            j !== endIndex &&
            (arrayData[i][j] === '.' || isNaN(parseInt(arrayData[i][j], 10))))
        ) {
          surroundedBySymbols = false;
          break;
        }
      }

      if (surroundedBySymbols) {
...
console.log('getNumbersNotSurroundedByDots',findNumbersSurroundedBySymbols(arrayData));
