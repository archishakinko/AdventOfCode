const text =`51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
...
function getNumberString(s, i) {
    for (const index in numberStrings) {
        const numString = numberStrings[index];
        if (s.substr(i, numString.length) === numString) {
...
function number2(s) {
    let i, ns;
    for (i = 0, ns = getNumberString(s, i); i < s.length && Number.isNaN(Number(s[i])) && !ns; i++, ns = getNumberString(s, i));
    let num = ns || Number(s[i]);
    if (Number.isNaN(num)) {
        return 0;
    }
    let j;
    ns = null;
    for (j = s.length - 1, ns = getNumberString(s, j); j > 0 && Number.isNaN(Number(s[j])) && !ns; j--, ns = getNumberString(s, j));
    num = num * 10 + (ns || Number(s[j]));
    console.log(`${num}`);
    return num;
}
console.log(lines.reduce((sum, line) => sum + number2(line), 0));